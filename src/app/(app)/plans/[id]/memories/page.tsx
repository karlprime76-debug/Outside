...
export default function MemoriesPage() {
  ...
  if (error) {
    return (
      <div className="p-4 max-w-3xl mx-auto text-center pt-16">
        <p className="text-[var(--os-muted)] mb-4">{error}</p>
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto space-y-6 pb-16">
      ...
      {/* Who came */}
      <div className="rounded-2xl border border-[var(--os-card-border)] bg-[var(--os-card)] p-4 sm:p-5 ">
        <div className="flex items-center gap-2 mb-4">
          <Camera className="h-4 w-4 text-outside-500" />
          <h3 className="text-sm font-bold text-[var(--os-fg)]">Qui est venu</h3>
          {checkedInCount > 0 && (
        ...
        <div className="flex flex-wrap gap-3">
          {participants.map((p) => (
            <div
              key={p.id}
              className="flex items-center gap-2 rounded-full border border-[var(--os-card-border)] bg-[var(--os-bg)] px-3 py-2"
            >
              <Avatar src={p.image} name={p.name} size="sm" />
              <span className="text-sm font-semibold text-[var(--os-fg)]">{p.name || "Anonyme"}</span>
      ...
      {highlight && (
        <div className="rounded-2xl border border-[var(--os-card-border)] bg-[var(--os-card)] p-4 sm:p-5 ">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h3 className="text-sm font-bold text-[var(--os-fg)]">Meilleur participant</h3>
          </div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-base font-bold text-[var(--os-fg)]">{highlight.name || "Anonyme"}</p>
              <p className="text-xs text-[var(--os-muted)]">{highlight.count} avis · {Math.round((highlight.score / highlight.count) * 100)}% positifs</p>
            </div>
          </div>
        </div>
      )}
      ...
      {polls.length > 0 && (
        <div className="rounded-2xl border border-[var(--os-card-border)] bg-[var(--os-card)] p-4 sm:p-5 ">
          <h3 className="text-sm font-bold text-[var(--os-fg)] mb-4">Sondages</h3>
          <div className="space-y-4">
            {polls.map((poll) => {
              const maxVotes = Math.max(...poll.options.map((o) => o.voteCount), 1);
              return (
                <div key={poll.id}>
                  <p className="text-sm font-semibold text-[var(--os-fg)] mb-2">{poll.question}</p>
                  <div className="space-y-1.5">
                    {poll.options.map((option) => {
                      ...
                      return (
                        <div key={option.id} className="relative rounded-lg bg-[var(--os-bg)] px-3 py-2">
                          <div
                            className="absolute inset-0 rounded-lg bg-outside-500/10 dark:bg-outside-500/15 transition-all"
                            style={{ width: `${pct}%` }}
                          />
                          <div className="relative z-10 flex items-center justify-between">
                            <span className="text-sm font-semibold text-[var(--os-fg)] flex items-center gap-1.5">
                              {option.label}
                              {isWinner && <Trophy className="h-3 w-3 text-amber-500" />}
                            </span>
                            <span className="text-xs font-bold text-[var(--os-muted)]">{option.voteCount} · {pct}%</span>
                          </div>
                        </div>
                      );
                    })}
      ...
      {recentMessages.length > 0 && (
        <div className="rounded-2xl border border-[var(--os-card-border)] bg-[var(--os-card)]  overflow-hidden">
          <div className="flex items-center gap-2 border-b border-[var(--os-card-border)] px-5 py-3">
            ...
            <h3 className="text-sm font-bold text-[var(--os-fg)]">Derniers messages</h3>
            <span className="ml-auto text-xs text-[var(--os-muted)]">{messageCount} message{messageCount > 1 ? "s" : ""}</span>
          </div>
              ...
              <div key={msg.id} className="flex items-start gap-3 p-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-[var(--os-fg)]">{msg.author.name || "Anonyme"}</span>
                    <span className="text-[10px] text-[var(--os-muted)]">{new Date(msg.createdAt).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}</span>
                  </div>
                  <p className="text-sm text-[var(--os-fg)] truncate">{msg.content}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="text-center pt-4">
        <p className="text-xs text-[var(--os-muted)]">Plan créé par {plan.creator.name || "Anonyme"}</p>
      </div>
    </div>
  );
}
...
function StatCard({ icon: Icon, label, value, color }: { icon: typeof Users; label: string; value: string; color: string }) {
  return (
    <div className="rounded-2xl border border-[var(--os-card-border)] bg-[var(--os-card)] p-4 text-center ">
      <Icon className={`h-5 w-5 mx-auto mb-1 ${color}`} />
      <p className="text-xl font-black text-[var(--os-fg)]">{value}</p>
      <p className="text-[10px] font-bold uppercase tracking-wider text-[var(--os-muted)]">{label}</p>
    </div>
  );
}
