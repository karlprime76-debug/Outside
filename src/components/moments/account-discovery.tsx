...
export function AccountDiscovery() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/api/friends/suggestions?limit=10");
        const data = await res.json();
        if (!cancelled) {
          setUsers((data.suggestions || []).slice(0, 8));
          setLoading(false);
        }
      } catch {
  ...
  return (
    <div className="py-4 px-2 border-b border-[var(--os-card-border)]">
      <p className="px-2 mb-2 text-xs font-bold text-[var(--os-muted)] uppercase tracking-wide">
        Comptes à découvrir
      </p>
