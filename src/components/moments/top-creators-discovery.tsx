...
export function TopCreatorsDiscovery() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch("/api/users/discover?type=creators&limit=8");
        ...
        {users.map((user) => (
          ...
          >
            <Link href={`/u/${user.username || user.id}`} className="flex flex-col items-center">
              ...
              {user.momentsCount && user.momentsCount > 0 ? (
                <p className="text-[10px] text-outside-500 font-semibold">
